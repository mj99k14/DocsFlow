...
※ 분류 규칙:
1. 문서가 특정 부서에 해당하면 그 부서를 포함하세요.
2. 문서 내용이 여러 부서에 걸쳐있으면 departments에 해당하는 부서 전부를 포함하세요.
3. "반려"는 위 목록의 어떤 실제 부서에도 해당하지 않을 때만 선택하세요.
   단순히 두 부서 이상이 해당된다는 이유로 반려를 선택하지 마세요.
"""
