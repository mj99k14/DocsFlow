...
**⑥ 非同期 AI 分析パイプライン**
文書をアップロードした瞬間にレスポンスを返し、AI 分析は `BackgroundTasks` でバックグラウンド処理します。
分析に数秒かかっても UI がブロックされず、ユーザーはすぐに次の操作へ移れます。
ダッシュボードは 5 秒ごとにポーリングして分析結果を自動反映します。

**⑦ DB から動的に取得した部署リストで AI 分類**
AI に渡す部署リストをコードにハードコードせず、毎回 DB から取得して Claude に渡します。
管理画面で部署を追加・削除するだけで、コード変更なしに AI の分類基準が即座に反映される設計です。
...
**⑥ 비동기 AI 분석 파이프라인**
문서를 업로드하는 즉시 응답을 반환하고, AI 분석은 `BackgroundTasks`로 백그라운드에서 처리합니다.
분석에 수 초가 걸려도 UI가 멈추지 않고, 대시보드는 5초마다 폴링해서 결과를 자동 반영합니다.

**⑦ DB 기반 동적 부서 분류**
AI에 전달하는 부서 목록을 코드에 하드코딩하지 않고 매번 DB에서 가져와 Claude에 전달합니다.
관리 화면에서 부서를 추가·삭제하면 코드 수정 없이 AI 분류 기준이 즉시 반영됩니다.
